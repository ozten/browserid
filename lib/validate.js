...
var types = {
  ...
  hostname: function (x) {
    /* Mostly the same as origin regex. See origin for details.
       Allowed value - default - indicates the default hostname should be used
    */
    if ('default' === x) return;
    var regex = /^(?=.{1,254}(?::|$))(?:(?!-)(?![a-z0-9\-]{1,62}-(?:\.|:|$))[a-z0-9\-]{1,63}\b(?!\.$)\.?)+$/i;
