...
exports.API_REGEX = new RegExp('^\/wsapi\/([^\/]*\/)?([^\/]+)$');

function checkCodeVersion(req, resp, next) {
  if (req.url.indexOf(WSAPI_PREFIX) === -1) return next();
  var purl = url.parse(req.url);
  var path = purl.pathname;
  var m = exports.API_REGEX.exec(path);
  if (!m || !m[1]) return codeCheckFailed(req, resp);
  var shaPortion = version() + '/';
  if (m[1] !== shaPortion) return codeCheckFailed(req, resp);

  req.url = ['/wsapi', m[2]].join('/');

  if (purl.search) req.url += purl.search;
  if (purl.hash) req.url += purl.hash;

  next();
  function codeCheckFailed (req, resp) {
    httputils.badRequest(resp, "Code version mis-match");
  }
}
