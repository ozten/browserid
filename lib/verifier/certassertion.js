...
function verify(assertion, audience, forceIssuer, successCB, errorCB) {
  // assertion is bundle
  var ultimateIssuer;
  logger.info('certassertion verify called forceIssuer=' + forceIssuer);
  jwcrypto.cert.verifyBundle(
    ...
    new Date(), function(issuer, next) {
      ...
      logger.info("jwcrypto callback with issuer=" + issuer);
	...
	logger.info('force issuer case forceIssuer=' + forceIssuer + " public key=" + publicKey);
